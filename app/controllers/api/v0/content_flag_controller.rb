class API::V0::ContentFlagController < API::V0::APIController
  ...
  def create
    ...
    if cflag.save
      #render :json => {:success => true, :flag => cflag.as_json(only: [:id, :user_id, :content_id])}, :status=> :created #201
      render :json => {:success => true, :flag => cflag.as_json}, :status=> :created #201
    else
